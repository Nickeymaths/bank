...
func TestTransferAPI(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorization(t, request, tokenMaker, beaserTokenType, user1.Username, util.DepositoryRole, time.Minute)
			},
			buildStubs: func(store *mockdb.MockStore) {
		...
		},
		{
			...
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorization(t, request, tokenMaker, beaserTokenType, user2.Username, util.DepositoryRole, time.Minute)
			},
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().GetAccount(gomock.Any(), gomock.Eq(account1.ID)).Times(1).Return(account1, nil)
		...
		},
		{
			...
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorization(t, request, tokenMaker, beaserTokenType, user3.Username, util.DepositoryRole, time.Minute)
			},
			buildStubs: func(store *mockdb.MockStore) {
