...
import (
	...
	mockwk "github.com/Nickeymaths/bank/worker/mock"
...
func (expected eqCreateUserTxParamsMatcher) Matches(x interface{}) bool {
	...
	err := util.IsCorrectPassword(expected.password, actualArg.HashedPassword)
	if err != nil {
		return false
	}
	...
	err = actualArg.AfterCreateCallback(expected.arg.Username)
	...
	}{
		{
			...
			buildStubs: func(store *mockdb.MockStore, taskDistributor *mockwk.MockTaskDistributor) {
				...
				taskDistributor.EXPECT().
					DistributeTaskVeriyEmail(gomock.Any(), payload, gomock.Any()).
					Times(1).
					Return(nil)
			},
			checkResponse: func(t *testing.T, res *pb.CreateUserResponse, err error) {
